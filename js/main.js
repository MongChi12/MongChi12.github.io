$(function(){
    ...
    const swiper = new Swiper(".mainSwiper",{
        ...
        on:{
            init:function(){
                // 모든 swiper-silde의 porgress-bar에서 클래스 제거
                $(".swiper-progress-bar").removeClass("animate");
                // 첫번째 swiper-silde에 해당하는 progress-bar에만 클래스 추가
    ...
    .to(".product-box .cont .txt2",{
        opacity:1, delay:3, duration:100
    });
    ...
    .to(".product-box .left-cover-box .cover3",{
        height:'50%', duration:100, delay:100
    });
    //위쪽 마스크 영역의 높이 50%
    ...
    .to(".product-box .left-cover-box .cover4",{
        width:'50%', duration:100, delay:100
    });
    //오른쪽 마스크 영역의 넓이 50%
    ...
    productAni.add([proAni26])
              ...
              .add([proAni2, proAni3, proAni4, proAni5, proAni7, proAni8, proAni9, proAni10, proAni11, proAni12, proAni13])
              .add([proAni14, proAni15, proAni16, proAni17])
              .add([proAni27, proAni18, proAni19, proAni20, proAni21, proAni22, proAni23, proAni24, proAni25]);


    //커스텀 커서 포인터(curtom-cursor)
    ...
    //문서(doucment)에 마우스무브 이벤트 설정
    $(".custom-cursor-area").on("mousemove", function(e){
        mouseX = e.pageX;
        mouseY = e.pageY
    });
    //custom-cursor-area 영역에 마우스를 올리면 커스텀 커서 나타남
    $(".custom-cursor-area").on("mouseenter", function(){
        scale.play();
        cursor.css('z-index',100);
    });
    //custom-cursor-area 영역에 마우스를 아웃하면 커스텀 커서 사라짐
    ...
    //마우스 스크롤을 내리면 header가 사라지고(addClass(active)), 마우스 스크롤을 올리면 header가 내려오도록 설정(removeClass(active))하는 함수 선언
    ...
    function headerHide(){
        ...
        //0.05초마다 스크롤 이벤트를 감지하여 화면이 이동중이고, body에 open클래스가 설정되어 있지 않다면 has_scroll함수를 호출하고, work_scroll변수의 값을 false로 만듬.
        ...
        function has_scroll(){
            ...
            //현재 scrollTop값이 last_scroll(0)보다 크면 header를 올려서 감추고, last_scroll(0)보다 크지 않으면 header를 내려서 보이게 함.
    ...
    $(window).on('scroll', function() {
        ...
        //viewPort화면 안에 section이 들어오면 body 의 배경색을 bg속성색으로 변경
        if ($section) {
